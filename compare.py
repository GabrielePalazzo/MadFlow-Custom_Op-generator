    ...
    def generate_integrand(lhewriter=None):
        ...
        def cross_section(xrand, n_dim=ndim, weight=1.0):
            ...
            for i, (matrix, model) in enumerate(zip(matrices, models)):
                ev = model.evaluate(alpha_s)
                smatrix = matrix.cusmatrix(all_ps, *ev)
                #tf.print(smatrix)
                smatrix2 = matrix.smatrix(all_ps, *ev)
                if not args.no_pdf:
            ...
            if args.pt_cut is not None:
                ...
                ret2 = tf.scatter_nd(idx, ret2, shape=out_shape)
