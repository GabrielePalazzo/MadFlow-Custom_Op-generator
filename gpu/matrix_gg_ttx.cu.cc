...
template <typename T>
__device__ void VVV1P0_1 (T* V2, T* V3, T COUP, double M1_, double W1_, T* V1);
template <typename T>
__device__ void FFV1_0 (T* F1, T* F2, T* V3, T COUP, T& out_final);
template <typename T>
__device__ void FFV1_1 (T* F2, T* V3, T COUP, double M1_, double W1_, T* F1);
template <typename T>
__device__ void FFV1_2 (T* F1, T* V3, T COUP, double M2_, double W2_, T* F2);
template <typename T>
__global__ void matrix (const double* all_ps, const double* hel, const double* mdl_MT, const double* mdl_WT, const T* GC_10, const T* GC_11, double* out_final, const int nevents);
...
void MatrixFunctor<GPUDevice, T>::operator()(
    const GPUDevice& d,const double* all_ps, const double* hel, const double* mdl_MT, const double* mdl_WT, const T* GC_10, const T* GC_11, double* out_final, const int nevents, const OpKernelContext* context) {
    ...
    matrix<T><<<numBlocks, blockSize, 0, d.stream()>>>(all_ps, hel, mdl_MT, mdl_WT, GC_10, GC_11, out_final, nevents);
