...
template <typename T>
void FFV1P0_3 (T* F1, T* F2, T COUP, double M3_, double W3_, T* V3);
template <typename T>
void matrix (const double* all_ps, const double* hel, const double* mdl_MT, const double* mdl_WT, const T* GC_11, double* out_final, const int nevents, const OpKernelContext* context);
...
struct MatrixFunctor<CPUDevice, T> {
  void operator()(const CPUDevice& d,const double* all_ps, const double* hel, const double* mdl_MT, const double* mdl_WT, const T* GC_11, double* out_final, const int nevents, const OpKernelContext* context) {
      matrix(all_ps, hel, mdl_MT, mdl_WT, GC_11, out_final, nevents, context);
  }
};
...
class MatrixOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Tensor& GC_11_tensor = context->input(4);
    ...
    MatrixFunctor<Device, COMPLEX_TYPE>()(context->eigen_device<Device>(), all_ps, hel, mdl_MT, mdl_WT, GC_11, out_final.data(), nevents, context);
  }
};
#define REGISTER_CPU(T)\
  REGISTER_KERNEL_BUILDER(\
      Name("Matrixuuxttx").Device(DEVICE_CPU).TypeConstraint<T>("T"),\
...
#define REGISTER_GPU(T)\
  ...
  REGISTER_KERNEL_BUILDER(\
      Name("Matrixuuxttx").Device(DEVICE_GPU).TypeConstraint<T>("T"),\
